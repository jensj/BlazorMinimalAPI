...
using BlazorMinimalApis.Endpoints.Pages.Shared.Layout;
...
public class XComponentBase : IComponent
{
    ...
    [CascadingParameter] private PageState PageState { get; set; }
    [Parameter] public List<ValidationError> Errors { get; set; } = new();
    protected string Message { get; set; }

    protected Type GetLayout(Type? preferredLayout = null)
    {
        if (IsHtmx())
            return typeof(NoLayout);
        else
            return preferredLayout ?? typeof(MainLayout);
    }
    protected bool IsHtmx()
    {
        return HttpContextAccessor.HttpContext != null && HttpContextAccessor.HttpContext.Request.Headers.Any(h => h.Key.Equals("HX-Request", StringComparison.InvariantCultureIgnoreCase));
    }
    ...
    void IComponent.Attach(RenderHandle renderHandle)
    {
        ...
        if (_renderHandle.IsInitialized) throw new InvalidOperationException($"The render handle is already set. Cannot initialize a {nameof(ComponentBase)} more than once.");
    ...
    public bool HasError<TValue>(Expression<Func<TValue>> expression)
    {
    ...
    {
        if (Errors is { Count: > 0 }) HasErrors = true;
